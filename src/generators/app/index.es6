...
class GitGenerator extends Base {
  ...
  searchUsers() {
    if (!this.user.git.email()) {
      console.log(chalk.red('Cannot find git email.  You should set: git config --global user.email "your_email@example.com" '));
    }
    return this.github.search.users({ q: this.user.git.email() + ' in:email' })
      .then(user => {
        ...
        if (user.total_count === 1) {
          ...
          return this.prompt(this.prompts.search)
          .then(answers => {
            if (answers.username) {
  ...
  authorization() {
    return new Promise((resolve, reject) => {
      this.prompt(this.prompts.authenticate)
        .then(answers => {
        ...
        .then(() => {
          ...
          return this.github.authorization.getAll({
            ...
            if (this.config.authorization.current) {
              ...
              this.prompt(this.prompts.authorization)
              .then(answers => {
                if (answers.token) {
  ...
  repoInfo() {
    return new Promise((resolve, reject) => {
      this.prompt(this.prompts.repoInfo)
        .then(answers => {
          let repository = find(this.config.repos, { name: answers.name });
          if (repository) {
